...
<body>
    ...
    <main class="px-2">
        <div class="flex flex-col text-xl gap-4">
            <p>
                Transaksi: Nomor//{{ $trans->id }}.
            </p>
            <p>
                Nama Peminjam: {{ $trans->user->name }}
            </p>
            <p>
                Nama Laboratorium: {{ $trans->user->phone }}
            </p>
            <p>
                Tanggal Meminjam: {{ $trans->borrow_date }}
            </p>
            <p>
                Tanggal Rencana Kembali: {{ $trans->return_date }}
            </p>
            <p>
                Jumlah Barang Keseluruhan: {{ $total }}
            </p>
        </div>
                ...
                <tbody>
                    @foreach ($tools as $index => $tool)
                        <tr>
                            ...
                            <td style="text-align: center">{{ $tool->amount }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
